...
<x-slot name="header">
    <h2 class="text-xl font-semibold">Update your product</h2>
</x-slot>

<main>
    <section aria-labelledby="update-product-heading">

        <h3 id="update-product-heading" class="text-lg font-medium mb-4">
            Update Product
        </h3>

        @if ($errors->any())
            <div role="alert" aria-labelledby="form-error-heading" class="mb-4 bg-yellow-50 dark:bg-yellow-900 p-3 rounded">
                <h4 id="form-error-heading" class="font-medium">
        ...
        <form action="{{ route('products.update', $product) }}" method="POST" novalidate class="max-w-3xl">        
        @csrf
        @method('PUT')
